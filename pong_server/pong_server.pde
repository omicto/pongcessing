...
float extMouseYright = 0;
float pExtMouseYright = 0;
float extMouseYleft = 0;
float pExtMouseYleft = 0;
...
void setup()
{
  ...
  extMouseYright = width/2;
  extMouseYleft = width/2;
...
void draw() 
{
  ...
  // Constrain paddle to screen
    ...
    float rightPaddle_Y = constrain(extMouseYright, paddle_height, height-paddle_height);
  ...
  {
    ball_dir *= -1;
    if(extMouseYright != pExtMouseYright) {
      dy = (extMouseYright-pExtMouseYright)/2.0;
      if(dy >  5) { dy =  5; }
      if(dy < -5) { dy = -5; }
    }
  }
  ...
  // Left paddle
  float leftPaddle_Y = constrain(extMouseYleft, paddle_height, height-paddle_height);
  ...
  {
    ball_dir *= -1;
    if(extMouseYleft != pExtMouseYleft) {
      dy = (extMouseYleft-pExtMouseYleft)/2.0;
      if(dy >  5) { dy =  5; }
      if(dy < -5) { dy = -5; }
    }
  } 
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if(theOscMessage.checkAddrPattern("/left") == true){
    ...
    moveLeftPaddleOnDifference(y);
    println("received Y:" + y + "ext: " + pExtMouseYright + " mo" + extMouseYright);
...
void moveRightPaddleOnDifference(float dif){
    ...
    if(dif > 0){
      ...
      extMouseYright += abs(dif) * speedFactor;
    }
    if(dif < 0){
      extMouseYright -= abs(dif) * speedFactor;
    }
    extMouseYright = constrain(extMouseYright, 0, 360);
...
void moveLeftPaddleOnDifference(float dif){
    ...
    if(dif > 0){
      ...
      extMouseYleft += abs(dif) * speedFactor;
    }
    if(dif < 0){
      extMouseYleft -= abs(dif) * speedFactor;
    }
    extMouseYleft = constrain(extMouseYleft, 0, 360);
